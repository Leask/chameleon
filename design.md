...
*   **協議回應 (總結與接受)**：我們**完全同意**這些最後一公里的批評。這不是小細節，而是決定實作能否互通與運營的關鍵。在 v12.0 版本中，我們：
    ...
    3. 對 Key Rollover Fallback 加入了「單一端點約束」與「本地決策快取」等抗噪聲機制，並明確了 `KEY_UPDATE` 各種門檻值的計算起點。
    4. 在恢復矩陣中引入了四個維度的**故障分類 (Fault Category)**（安全、配置、協議、實作），並為連續 Auth 失敗增加了避險中斷規則，防止對控制面發起雪崩請求。
    至此，協議的所有核心邊界均已無死角。