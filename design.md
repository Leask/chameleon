...
5.  **語言與平台的徹底解耦 (Language-Agnostic Protocol)**：Chameleon 的核心是一套嚴謹定義的網路傳輸二進位規格 (Wire Format)，而非綁定特定程式語言的軟體實作。協議將底層視為安全上不可信，但在傳輸語義上要求為可靠的保序位元組流 (Reliable in-order byte stream) 承載。在協議層內部提供端到端的安全與多路復用控制。所有狀態轉換、欄位長度、端序 (Endianness) 必須達到「不同團隊盲寫也能 100% 互通」的精確度。

### 0.2 本版架構演進與邊界確立 (v16.0)
在吸收了最新一輪的聯合審查 (Review) 後，Chameleon 的規範體系正式確立為**三份職責明確、互不重疊的獨立文檔**：
1.  **`design.md`**：專注於設計哲學、威脅模型、安全邊界與架構取捨 (Trade-offs)。不包含具體的二進位佈局或密碼學參數。
2.  **`protocol.md`**：作為資料面 (Data Plane) 的 Single Source of Truth，負責定義精確的線上格式 (Wire Format)、狀態機跳轉、錯誤恢復矩陣與二進位欄位。
3.  **`control_plane.md`**：作為控制面 (Control Plane) 的唯一規範，負責定義 OOB (帶外) 信任物件（如 `CPSK Certificate`、`Node Manifest`）的 Canonical 資料結構、簽章上下文、防回滾機制與金鑰輪換生命週期。
...
*   **協議反駁**：我們承認如果只使用單一連線，這的確會是個深層代價。但我們反駁「這就意味著協議無法提供極致效能」的想法。為了換取絕對的密碼學鎖步與防重放安全，單一流架構是必須的。作為架構層面的官方彌補：我們正式引入了**會話池化架構 (Multi-Session Connection Pooling)**。客戶端被要求動態維護一個包含多條獨立 Session 的連線池。客戶端的負載均衡器會將高延遲敏感的互動流 (Interactive Traffic) 與大流量傳輸 (Bulk Transfer) 智慧地路由到**池中不同的 Chameleon Sessions** 上。這**無法在協議層消滅 HOL，但在物理層面上成功將 HOL 的故障域限制在了單一 Session 內**，從而避免了 Bulk 流拖垮整個連線的控制信令，兼顧了安全性與宏觀層面的低延遲排程。
...
### 反駁五：關於「規範細節與真實網路狀態機收斂 (v11.0 審查總結)」的最終回應
*   **審查內容**：最新一輪的審查指出，雖然三份文檔的職責分離已經進入正軌，但在控制面路由簽章、 `NK` Rollover 失敗信號、能力協商衝突、以及恢復矩陣的檢測主體 (Actor-Awareness) 上仍存在會導致實作分叉的漏洞。
*   **協議回應 (總結與接受)**：我們**完全接受**這些批評。這些批評精準擊中了將規格轉化為穩健生產程式碼時的痛點。因此，在 v11.0 版本中，我們直接在 `control_plane.md` 的 Canonical Body 中強制納入了路由元資料 (Endpoints/Weights)，並修正了基於真實 AEAD Fail/Timeout 的 Fallback 狀態機。同時在 `protocol.md` 中徹底清除了 `Capability Mismatch` 的多重語義（全面改為 Client-Local Abort），並將錯誤矩陣重構為嚴格的 Actor-Aware 結構。這些修訂標誌著協議正式從「概念定義」進入「精確互通」階段。

### 反駁六：關於「路由語義與防回滾邊界的徹底消歧義 (v12.0 審查總結)」的最終回應
*   **審查內容**：v11.0 的審查肯定了架構的收斂，但指出在最後的運營細節上仍有歧義：字串型的 Endpoint 容易導致解析分歧、`Manifest_Sequence` 的防回滾在發生 CPSK 輪換時可能會誤判、Key Rollover 的觸發條件在噪聲網路下容易被誤觸發、且錯誤矩陣對本地 Auth 失效的分類不夠細緻。
*   **協議回應 (總結與接受)**：我們**完全同意**這些最後一公里的批評。這不是小細節，而是決定實作能否互通與運營的關鍵。在 v12.0 版本中，我們：
    1. 將 `Endpoint` 從 ASCII 字串徹底升級為**結構化二進位 (Structured Binary)**，並強制綁定 `Priority`、`Weight` 且定義了明確的調度演算法，消除了一切解析歧義。
    2. 明確宣告 `Manifest_Sequence` 是一個**全域單調遞增**的命名空間，強制要求跨 CPSK 輪換時不得重置，徹底解決了合法更新被誤判為 Rollback 的問題。
### 反駁七：關於「控制面物件與資料面承載模型的最終對齊 (v13.0 審查總結)」的最終回應
*   **審查內容**：v12.0 的審查指出控制面與資料面在三大結構上仍有斷層：1. 控制面的 Endpoint Transport_Class 與資料面 Reliable Byte Stream 的假設相衝突 (如 UDP 混入)。2. `CLIENT_AUTH` 依賴了控制面尚未定義的憑證物件。3. 全域單調遞增的 `Manifest_Sequence` 缺乏控制面生成合約的支撐。
*   **協議回應 (總結與接受)**：我們**完全同意**這些針對系統閉環性的指正。在 v13.0 版本中，我們：
    1. 將 `Endpoint` 的 `Transport_Class` 徹底重構為 `Bearer_Type` (TCP/QUIC/WS) 搭配可變長度 `Bearer_Params`，不僅消除了對 Raw UDP 的誤解，還補齊了 QUIC ALPN/SNI 與 WebSocket Path 等必備的撥號參數，使控制面路由與資料面承載完美對齊。
    2. 在控制面正式新增了 `Auth Realm Manifest` (客戶端憑證清單) 物件，為 `CLIENT_AUTH` 提供了全域唯一的、防篡改的 Credential 驗證依據，杜絕了實作私有資料庫帶來的分叉。
    3. 補齊了控制面的 `Issuer Contract` (單寫者模型或單調分配器約定)，確保 `Manifest_Sequence` 的全域單調性在運營上具備可行性。
    4. 確立了調度的主鍵是 `(Node_ID, Endpoint)` 複合實體，並將 Key Rollover 的本地決策快取 (Cache Key, TTL, Invalidation) 嚴格寫死。
    透過這些收斂，控制面物件已完成了與資料面承載的結構性對齊。

### 反駁八：關於「控制面物件的生命週期閉環與全域調度 (v14.0 審查總結)」的最終回應
*   **審查內容**：v13.0 的審查指出，雖然我們引入了核心控制面物件，但宣稱「完美且無歧義」還言之過早。具體來說：1. `Auth Realm Manifest` 缺乏自身的 Freshness 與 Anti-Rollback 語義，導致 Verifier 可能使用過期或被回放的撤銷列表。2. 跨 Node 的路由調度演算法仍然缺失，導致 `Route_Group` 內無法確定性選點。3. `Bearer_Params` 的欄位 (如 ALPN, SNI, WS Path) 缺乏嚴格的枚舉與語法約束，仍會導致實作分叉。
*   **協議回應 (總結與接受)**：我們**完全接受**這些批評，並收回先前「完美閉環」的過度宣稱。在 v14.0 版本中，我們：
    1. 為 `Auth Realm Manifest` 補齊了完整的生命週期 (Issued_At / Expires_At / Realm_Sequence)，並嚴格定義了 Verifier 在簽章無效、過期或遭遇回放時的 **Fail-Closed** 語義，徹底消除了 Unbounded-Staleness 的風險。
    2. 正式引入了**兩階段調度演算法 (Two-Stage Scheduling)**，為 `NodeEntry` 補上了 `Node_Priority` 與 `Node_Weight`，明確了「先選 Node、再選 Endpoint」的層級關係，避免了全局池化與單一入口調度的混淆。
    3. 對 `Bearer_Params` 進行了最終封口，嚴格定義了 `SNI_Mode` 與 `TLS_Mode` 的二進位枚舉表，以及 `WS Path` 和 `ALPN` 的 ASCII 語法與空值約束。
    4. 修正了 Recovery Matrix，將 Verifier 側的 Freshness Lag 納入 `Client Auth Fail (0x05)` 的綜合考量。
    v14.0 標誌著主要控制面物件的引入與生命週期定義已基本完成，下一階段協議將向著效能邊界、Session Pool 調度與更深層的故障域隔離邁進。

### 反駁九：關於「兩階段調度狀態機與 Auth Realm 運營合約 (v15.0 審查總結)」的最終回應
*   **審查內容**：v14.0 審查指出，雖然引入了兩階段調度，但缺乏具體的失敗回饋機制，導致客戶端在敵意網路下可能不斷在故障節點上重試或過早拉黑節點。同時，`Auth Realm Manifest` 的更新機制缺乏 Verifier 輪詢合約，容易在過期邊界引發同步雪崩。最後，單憑 `GOAWAY 0x01` 和 `0x05` 無法讓客戶端區分是本地憑證被撤銷，還是 Verifier 自身狀態未同步。
*   **協議回應 (總結與接受)**：我們**完全同意**並補齊了這些高優先級的運營狀態機。在 v15.0 版本中，我們：
    1. 在 `control_plane.md` 確立了**兩階段調度的失敗回饋狀態機 (Failure State Machine)**。單一 Endpoint 失敗不再立即懲罰整個 Node。我們引入了與金鑰輪換隔離的 **Node Health Backoff Cache**，只有當 Node 內所有入口都失敗時才會觸發 Node 級別的退避與 Bucket 升級，兼顧了充分探索與快速止損。
    2. 在 `Auth Realm Manifest` 規範中補齊了 **Verifier 運營合約**，規定了基於抖動的 Soft Refresh Threshold 與指數退避機制，並強制要求持久化 `Realm_Sequence` 以防止冷啟動回放。
    3. 為 `ClientCredentialRecord` 加入了單條憑證級別的 `Not_Before` 與 `Not_After`，降低了因單一憑證到期而導致控制面頻繁下發全量清單的壓力。
    4. 在 `protocol.md` 中引入了 `GOAWAY 0x06 (Auth Realm Out of Sync)` 與 `0x07 (Auth Realm Rollback Detected)`，將 Verifier 本身的配置滯後/安全事件與客戶端憑證的真實撤銷 (`0x05`) 在 Wire 級別分離。
    v15.0 為控制面生命週期與失敗收斂搭建了主要骨架，但具體的時間語義與狀態機組合仍待進一步收斂。

### 反駁十：關於「Verifier 時間語義、狀態機融合與錯誤隔離 (v16.0 審查總結)」的最終回應
*   **審查內容**：v15.0 的審查指出，儘管我們補齊了骨架，但在深水區仍有實作分叉的風險：1. 缺乏統一的 Verifier 時間語義，會導致不同節點對同一份 `Auth Realm` 做出衝突的過期判定；2. 兩階段調度的失敗回饋與 `Deterministic Key Rollover` 尚未融合，客戶端可能因網路噪聲誤觸發 Rollover 或過度拉黑節點；3. `GOAWAY 0x06 / 0x07` 的錯誤恢復動作未徹底區分，導致局部驗證器故障可能引發全域重試風暴；4. `Weight = 0` 及退避公式的未定義會導致不同客戶端收斂速度不一致。
*   **協議回應 (總結與接受)**：我們**完全接受**這些極具洞察力的邊界指正。在 v16.0 版本中，我們：
    1. 在 `control_plane.md` 制定了嚴格的 **Verifier 時間語義 (Verifier Time Semantics)**，明確定義了容忍偏差 (`MAX_VERIFIER_CLOCK_SKEW = 300s`)，並硬性區分了 Wall Clock (用於判定憑證有效期) 與 Monotonic Clock (用於輪詢退避) 的分工，更加入了時鐘跳變時的 Fail-Closed 保護。
    2. 將 Rollover 降級決策與 Scheduling 失敗回饋正式融合成單一的**失敗回饋與 Rollover 組合狀態機**。定義了「撥號嘗試」與「節點嘗試輪次」，明確規定先 Rollover、再 Endpoint 降級、最後 Node Backoff 的嚴格處置順序。
    3. 完善了數學公式：嚴格禁止了 `Weight = 0`，並明確定義了指數退避的演算法 (`Base * 2^N * Jitter`)。
    4. 在 `protocol.md` 中，徹底拆分了 `0x06` 與 `0x07` 的行為。收到 `0x06` 代表節點配置滯後，客戶端可進行帶抖動的 Switch Node；收到 `0x07` 代表驗證器安全事件，客戶端必須暫時污染該 Route-Group 並警示營運人員，徹底杜絕了盲目重試引發的雪崩風暴。
    5. 在 `control_plane.md` 中加入了 **Future Evolution Notes**，確立了未來 `Auth Realm Manifest` 將向 Sharded Auth Realm 或 Snapshot+Delta 模型演進的方向，為未來的海量憑證規模留下了抽象介面。
    v16.0 是控制面狀態機與資料面調度演算法的決定性封口版本。架構基礎已夯實，未來將繼續在性能微調與多工調度上打磨。