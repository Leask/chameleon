...
Chameleon 採用嚴格的控制面與資料面分離架構，並透過短期憑據解決邊緣節點的信任問題。

### 1.1 控制面與單向信任分發 (Control Plane & Trust Distribution)
*   **職責**：節點發現、憑證簽發、路由策略下發、版本治理。
*   **信任假設**：Root CA 絕對離線。控制面負責為全球的邊緣節點簽發**紀元憑證 (Epoch Cert)**。
*   **解決 Responder Key 矛盾**：在 Noise `NK` 模式中，伺服器需要一個「靜態公鑰 (Static Key)」。為了貫徹「邊緣節點不可信且短命」的原則，這個 Server Static Key **絕對不是**長期存在於邊緣節點的高價值秘密。相反，它是邊緣節點在啟動時隨機生成的**短期密鑰對 (Short-lived Keypair，例如 24 小時壽命)**。控制面透過 Epoch Cert 對這個短期公鑰進行簽名。客戶端透過驗證 Epoch Cert 的控制面簽名，來信任這個短期的 Server Static Key。

### 1.2 資料面與客戶端授權 (Data Plane & Client Auth)
*   **職責**：加密、多路復用、流控、錯誤恢復。
*   **客戶端身份驗證的解耦 (Decoupled Client Auth)**：為了追求握手階段的絕對零特徵 (Zero-Signature)，我們**刻意不在 Noise 握手層包含任何 Client Authentication 資料**。Noise 握手僅用於建立安全的匿名加密隧道。真正的客戶端身份驗證與設備撤銷檢查，發生在握手成功後的第一個加密 Channel 中（由上層的 Auth Frame 負責，細節見 `protocol.md`）。這確保了未經授權的掃描器連第一層加密隧道都無法建立。

### 1.3 握手重放防護的架構取捨 (Handshake Replay Trade-off)
*   **取捨聲明**：Chameleon 協議在 Record Layer (Post-Auth) 提供絕對的防重放保護 (Strict Lockstep Sequence)。然而，在握手階段 (Pre-Auth)，由於伺服器設計為無狀態 (Stateless) 且不依賴時鐘，我們**主動接受「握手請求可被重放」的風險**，將其視為威脅模型之外 (Out of Threat Model)。
*   **緩解措施**：重放合法的 Client 首包只會導致伺服器消耗極少量的 CPU 進行 DH 計算並回覆第二包，攻擊者無法解密後續流量，也無法劫持連線。營運層可透過簡單的 IP Rate Limiting 來緩解此類資源消耗攻擊。
...
### 反駁三：關於「單一流 (Single Stream) 多路復用會重新引入 HOL」的質疑
*   **質疑內容**：將所有邏輯通道塞入單一可靠位元組流，會導致任一底層丟包重傳卡住整個會話，破壞了「極致效能」的承諾。
*   **協議反駁**：我們承認並**主動接受這個代價**。為了換取絕對的密碼學鎖步 (Strict Lockstep) 與防重放安全，我們放棄了跨流的 HOL 避免能力。作為架構層面的彌補與**官方部署建議**：我們強烈建議客戶端/路由層**將高延遲敏感的互動流 (Interactive Traffic) 與大流量傳輸 (Bulk Transfer) 拆分到不同的 Chameleon Sessions 中**，利用底層傳輸 (如建立多條 TCP 連線或多個獨立的 QUIC Streams) 來實現物理級別的流量隔離，而不是將希望完全寄託在單一 Session 內部的邏輯排程上。
