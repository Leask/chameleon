...
* **Signer_Key_ID (強制作為查找鍵)**: 必須 (MUST) 為 `Public_Key` 進行 `BLAKE2s` 雜湊後截取的前 8 個 Bytes。
...
2. **平滑金鑰輪換算法 (Deterministic Key Rollover)**:
   ...
   * 若 `Next_Not_Before <= Now < Current_Not_After` (重疊視窗 Overlap Window)：
     * 預設依舊使用 `Current_Static_Pubkey`。
     * **僅當**本輪連線因為 `Responder Key Mismatch` 失敗時，才允許對同一 `Node ID` 使用 `Next_Static_Pubkey` 發起重試 (Fallback)。同一輪撥號最多允許一次 Current -> Next 的切換，禁止無限抖動。
